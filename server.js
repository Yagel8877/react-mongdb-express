...
app.get('/express_backend', (req,res) => {
  res.status(200).send({express: 'heyyya'})
})
...
app.post('/login', async (req,res)=>{
  ...
  if(await bcrypt.compare(req.body.password, user.password)){
      ...
      mongoose.disconnect().then(res.send('not allowed')).then(console.log('disco1'))
      
    }
  }
    catch{
      mongoose.disconnect().then(res.send('username doesnt exists or othrt')).then(console.log('disco2'))
  }
